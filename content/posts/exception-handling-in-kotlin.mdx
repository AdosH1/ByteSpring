...
description: The use of exceptions vs. result types is a controversial topic that has sparked strong feelings on both sides of the fence. Kotlin's unique history has subtlely shifted its developers to program a certain way. In this article we will discuss the history from which Kotlin was created from and how the Kotlin team views exception handling in the language. 
date: "2023-12-09T00:00:00Z"
---

Kotlin is a modern programming language with its first stable release dating back to February 2016.
It began development in 2010 as developers found a need to build a language that was _concise, elegrant and expressive, but also **interoperable with Java**_.
...
In this article, we'll breakdown my thoughts and takeaways from [Kotlin and Exceptions by Roman Elizarov](https://elizarov.medium.com/kotlin-and-exceptions-8062f589d07), the project lead for Kotlin programming @ JetBrains.
...
For problems where its less clear whether it's an input or a program logic error, Kotlin provides an API for both cases.  
...
```
This code now idicates an error through its _result value (using null)_, which will play well with the rest of the Kotlin language.
...
Roman elaborates:
> If there are multiple error conditions, then create a sealed class hierarchy to represent various results of your function. Kotlin has all those needs covered by design, including a powerful when expression.
...
## Building Applications the Kotlin Way 
As a subscriber to [Hexagonal Architecture](https://en.wikipedia.org/wiki/Hexagonal_architecture_(software)), the model fits very well with Roman's recommendations.
The core domain should be pure with result types, pushing all I/O _(and thereby, exception handling)_ out to the _**ports and adapters**_.

The ports and adapters will all have their own top level handling of exceptions which will convert into result types for our domain, or separately handled for its use case _eg. 500 error for an API_. 
<img src="/exceptions-in-kotlin/pure-hex-arch.png" width="359" height="202" alt="Image shows an example REST domain setup." style={{borderRadius:'15px', display:'block', marginLeft: 'auto', marginRight: 'auto', width: '50%'}} />
...
2. There is no actual enforcement to unravel result type, failure handling errors  will throw exceptions!
<img src="/exceptions-in-kotlin/default-result-types.png" width="718" height="404" alt="Image shows an example REST domain setup." style={{borderRadius:'15px'}} />

So  what should to use instead? _I can't say._  
I'm still exploring options and have been having some success with [Arrow.kt](https://arrow-kt.io/), 
though I've not used it extensively enough in a production environment to provide any advice.
