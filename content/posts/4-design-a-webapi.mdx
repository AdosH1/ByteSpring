...
description: A practical, high level approach to designing a webapi using best practices such as Domain Driven Design (DDD), the Repository Pattern, Command Query Response Segregation (CQRS) and Hexagonal architecture
date: "2023-08-24"
---
...
### REST Domain Example
#### At a high level
Let's take a look at the REST domain as an example, say we have a service that _processes dog photos and adds a hat to each dog identified._ 
We've built the system but want to host this service online, so we'll need to _implement a layer to handle web requests._ 

This domain will _handle an **application's ability to receive and serve web requests, and nothing more**._ 
So _what responsibilities would our REST domain have?_
